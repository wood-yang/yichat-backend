...
import com.wood.yupao.model.request.*;
import com.wood.yupao.model.vo.TeamUserVO;
import com.wood.yupao.service.TeamService;
...
public class TeamController {
    ...
    public BaseResponse<List<TeamUserVO>> listTeams(TeamQuery teamQuery, HttpServletRequest request) {
        ...
        if (teamList.isEmpty()) {
            return ResultUtils.success(teamList);
//            throw new BusinessException(ErrorCode.NULL_ERROR, "没有队伍");
        }
        // 2. 判断当前用户是否已经加入队伍
        final List<Long> teamIdList = teamList.stream().map(TeamUserVO::getId).collect(Collectors.toList());
        ...
        try {
            ...
            System.out.println("判断用户是否加入集合出问题了");
        }
        // 3. 查询已加入队伍的用户信息（人数）
        QueryWrapper<UserTeam> userTeamJoinQueryWrapper = new QueryWrapper<>();
        userTeamJoinQueryWrapper.in("teamId", teamIdList);
